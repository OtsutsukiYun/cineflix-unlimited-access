  ...
  return (
    <div className="group relative shrink-0 overflow-hidden rounded-2xl border border-white/15 bg-gradient-to-b from-surface/90 to-background w-[150px] sm:w-[172px] md:w-[185px] transition-all duration-500 ease-out hover:-translate-y-2.5 hover:scale-[1.04] hover:border-blue-400/90 hover:shadow-[0_14px_40px_rgba(249,115,22,0.45)] cursor-pointer">
      ...
      <div className="absolute inset-0 z-30 flex items-center justify-center opacity-0 transition-all duration-300 group-hover:opacity-100 bg-black/35 backdrop-blur-[2px]">
        <div className="relative flex size-12 items-center justify-center rounded-full bg-gradient-to-tr from-primary to-accent text-white shadow-[0_0_30px_rgba(249,115,22,0.95)] transform scale-75 transition-transform duration-300 group-hover:scale-100 border border-white/40">
      ...
      {/* BADGE DE TAG */}
      {item.tag && (
        <span className="absolute top-2.5 left-2.5 z-40 max-w-[70%] truncate inline-flex items-center gap-1 rounded-full border border-blue-400/40 bg-blue-950/85 backdrop-blur-md px-2 py-0.5 text-[9px] font-extrabold tracking-wider text-blue-200 uppercase shadow-sm">
          {item.tag}
        </span>
      )}
      ...
      <div className="absolute inset-x-0 bottom-0 z-40 p-3 sm:p-4">
        <p className="truncate text-xs sm:text-sm font-black text-white drop-shadow-md transition-colors group-hover:text-blue-200">
          {item.title}
        </p>
        ...
        <div className="mb-3 flex items-end justify-between gap-4 px-1 sm:px-2">
          <div>
            <div className="flex items-center gap-2 sm:gap-2.5">
              <Icon className="size-5 sm:size-6 text-orange-300 shrink-0 animate-pulse drop-shadow-[0_0_20px_rgba(232,121,249,0.9)]" />
